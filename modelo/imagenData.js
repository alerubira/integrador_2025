...
class ImagenData{
        ...
        try {
            ...
            await connection.beginTransaction();
             ...
             const [imagenResult] = await connection.execute(
                'INSERT INTO `imagen`(`id_album_personal`,`url_imagen`, `fecha_creacion_imagen`, `titulo_imagen`,`caption_imagen`,`id_visibilidad`,`activo_imagen`) VALUES (?,?,?,?,?,?,?)',
                [idAlbum,imagen.urlImagen, imagen.fechaCreacion, imagen.tituloImagen, imagen.captionImagen, imagen.idVisivilidad, imagen.activoImagen],
            );
    /*
    ...
    static async buscarImagenesPorIdAlbumPersonal(idAlbumPersonal) {
         query = 'SELECT img.id_imagen,img.url_imagen,img.fecha_creacion_imagen,img.titulo_imagen,img.caption_imagen,img.id_visibilidad,vi.titulo_visibilidad,img.activo_imagen FROM imagen img JOIN album_personal a_p on img.id_album_personal=a_p.id_album_personal JOIN visibilidad vi on img.id_visibilidad=vi.id_visibilidad WHERE a_p.id_album_personal=?;'
    ...
    static async buscarImagenesPublicasPublicas(){
        query='SELECT img.id_imagen,img.url_imagen,img.fecha_creacion_imagen,img.titulo_imagen,img.caption_imagen,pe.img_perfil,pe.nombre_perfil,pe.id_perfil,per.nombre_persona,per.apellido_persona,ap.id_tags FROM imagen img JOIN album_personal ap ON img.id_album_personal=ap.id_album_personal JOIN perfil pe on ap.id_perfil_personal=pe.id_perfil JOIN persona per ON pe.id_persona=per.id_persona WHERE (img.id_visibilidad=1||img.id_visibilidad=2)&&img.activo_imagen=1 ORDER BY img.fecha_creacion_imagen DESC;'
        return await consulta1(query);
    }
    static async buscarImagenesParaSeguidores(idSeguidor){
        query='SELECT img.id_imagen,img.url_imagen,img.fecha_creacion_imagen,img.titulo_imagen,img.caption_imagen,pe.img_perfil,pe.nombre_perfil,pe.id_perfil,per.nombre_persona,per.apellido_persona FROM imagen img JOIN album_personal ap ON img.id_album_personal=ap.id_album_personal JOIN perfil pe on ap.id_perfil_personal=pe.id_perfil JOIN persona per ON pe.id_persona=per.id_persona JOIN album_seguidor als on als.id_perfil_seguido=pe.id_perfil WHERE (img.id_visibilidad=3)&&(img.activo_imagen=1)&&(als.id_perfil_seguidor=?) ORDER BY img.fecha_creacion_imagen DESC;'
    ...
    static async buscarImagenesEtiquetadasPersonal(idSeguidor){
        ...
        query='SELECT img.id_imagen,img.url_imagen,img.fecha_creacion_imagen,img.titulo_imagen,img.caption_imagen,pe.img_perfil,pe.nombre_perfil,pe.id_perfil,per.nombre_persona,per.apellido_persona FROM imagen img JOIN album_imagen ai ON img.id_imagen=ai.id_imagen JOIN album_seguidor als on als.id_album_seguidor=ai.id_album_seguidor JOIN perfil pe on als.id_perfil_seguido=pe.id_perfil JOIN persona per ON pe.id_persona=per.id_persona WHERE(img.activo_imagen=1) &&(als.id_perfil_seguidor)=? ORDER BY img.fecha_creacion_imagen DESC;'
        return await consulta1(query,idSeguidor);
    }
